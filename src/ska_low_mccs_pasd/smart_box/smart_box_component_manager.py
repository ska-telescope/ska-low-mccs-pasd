...
class Port:
    ...
    def set_desire_on(self, task_callback: Optional[Callable]) -> None:
        ...
        """
        self.logger.info(
            "Port desired on. To be turned on when smartbox is on."
        )
        self.desire_on = True
    ...
    def turn_on(self, task_callback: Optional[Callable] = None) -> None:
        ...
        command_tracker = next(
            item
            for item in [task_callback, self._task_callback]
            if item is not None
        )
...
class _SmartBoxProxy(DeviceComponentManager):
    ...
    def __init__(
        ...
        logger: logging.Logger,
        max_workers: int,
        smartbox_communication_state_callback: Callable[
...
class _FndhProxy(DeviceComponentManager):
    ...
    def __init__(
        ...
        logger: logging.Logger,
        max_workers: int,
        fndh_communication_state_callback: Callable[
            [CommunicationStatus], None
        ],
...
class SmartBoxComponentManager(
    ...
    def __init__(
        ...
        self.logger = logger
        self.ports = [
            Port(self.turn_on_port, port, logger)
    ...
    def _pasdbus_state_changed(
        ...
        if self._component_state_callback is not None:
            self._component_state_callback(
                **state_change, fqdn=self._pasd_fqdn
            )
    ...
    def _fndh_state_changed(
        ...
        if self._component_state_callback is not None:
            self._component_state_callback(
                **state_change, fqdn=self._fndh_fqdn
            )
        ...
        try:
            if self._fndh_port:
                if self._smartbox_proxy is None:
                    raise ValueError(
                        f"Power on smartbox '{self._fndh_port} failed'"
                    )
    ...
    def _off(
        ...
        try:
            if self._fndh_port:
                if self._smartbox_proxy is None:
                    raise ValueError(
                        f"Power off smartbox '{self._fndh_port} failed'"
                    )
