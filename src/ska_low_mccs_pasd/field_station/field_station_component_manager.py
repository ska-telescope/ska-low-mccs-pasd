...
class FieldStationComponentManager(TaskExecutorComponentManager):
    ...
    def __init__(
        ...
        self._antenna_mapping: dict[str, tuple[str, int]] = {}
        self._all_masked = False
        self._antenna_mask: dict[str, bool] = {}
        ...
        if _smartbox_proxys:
            ...
            for smartbox_trl in smartbox_names:
                ...
                self._smartbox_name_number_map.update({smartbox_trl: smartbox_name})
                smartbox_count += 1
    ...
    def _update_mappings(
        ...
        antenna_mapping_logical: dict[str, tuple[str, int]] = {}
        ...
        for antenna_id, antenna_config in reference_data["antennas"].items():
            ...
            self.antenna_powers[antenna_id] = PowerState.UNKNOWN
    ...
    def _on_port_power_change(
        ...
        if event_value is None:
            ...
            return
        if smartbox_trl not in self._smartbox_name_number_map.keys():
            self.logger.error(
        ...
        for antenna_id, (
            antennas_smartbox_id,
            smartbox_port,
        ) in self._antenna_mapping.items():
            smartbox_name = self._smartbox_name_number_map[smartbox_trl]
            if antennas_smartbox_id == smartbox_name:
                if antenna_id in self.antenna_powers:
                    port_index = smartbox_port - 1
                    if 0 <= port_index < len(port_powers):
                        self.antenna_powers[antenna_id] = port_powers[port_index]
                        ...
                        self.logger.warning(
                            ...
                            f"for antenna {antenna_id}"
                        )
                else:
                    self.logger.warning(
                        f"Warning: Antenna {antenna_id} not " "found in antenna powers"
                    )
    ...
    def smartbox_state_change(
        ...
        """
        self._smartbox_power_state[smartbox_name] = power
    ...
    def _on(  # noqa: C901
        ...
        self.logger.error("on 1")
        if not ignore_mask and self._all_masked:
            self.logger.error("on 2")
            msg = (
            ...
            return
        self.logger.error("on 3")
        if task_callback:
            task_callback(status=TaskStatus.IN_PROGRESS)

        try:
            self.logger.error("on 4")
        ...
        self.logger.error("on 9")
        if task_callback:
            self.logger.error("on 10")
    ...
    def _turn_on_station(  # noqa: C901
        ...
        """
        self.logger.error("tunr on station 1")
        failure_log = ""
        ...
        self.logger.error(f"masked_smartbox_ports == {masked_smartbox_ports}")
        self.logger.error(f"masked_fndh_ports == {masked_fndh_ports}")
        self.logger.error("tunr on station 2")
        ...
        self.logger.error(f"tunr on station 4, json arg == {json_argument}")
        [set_fndh_port_powers_result], _ = self._fndh_proxy._proxy.SetPortPowers(
            json_argument
        )
        self.logger.error("tunr on station 5")
        if set_fndh_port_powers_result == ResultCode.QUEUED:
            self.logger.error("tunr on station 6")
            t1 = time.time()
            self.logger.info(
            ...
            timeout -= time_taken
            self.logger.error("tunr on station 7")
            ...
            if all(result == ResultCode.QUEUED for result in results):
                ...
                self.logger.error("tunr on station 11")
                self.wait_for_antenna_powers(desired_antenna_powers, timeout)
                self.logger.error("tunr on station 12")
                t4 = time.time()
    ...
    def _calculate_desired_antenna_powers(
        ...
        for antenna_id, (smartbox_id, smartbox_port) in self._antenna_mapping.items():
            for masked_smartbox_id, masked_ports in masked_smartbox_ports.items():
                if masked_smartbox_id == smartbox_id:
                    if smartbox_port not in masked_ports:
                        desired_power[antenna_id] = desired_power_state
    ...
    def wait_for_fndh_port(  # noqa: C901
        ...
        """
        self.logger.error(f"WAIT FOR FNDH PORT desired == {desired}")

        def _fndh_ports_match_desired(
            current_state: list[Optional[bool]], desired_state: list[Optional[bool]]
        ) -> bool:
            self.logger.error(
                f"current_state == {current_state}, desired_state == {desired_state}"
            )
    ...
    def wait_for_antenna_powers(  # noqa: C901
        ...
        def antenna_power_as_desired(
            ...
            for antenna_id, desired_power in desired_antenna_powers.items():
                if desired_power is not None:
                    if current_state[antenna_id] != desired_power:
                        return False
            return True
    ...
    def _get_masked_smartbox_ports(
        ...
        # Smartbox_id : [List of masked ports on that smartbox]
        masked_smartbox_ports: dict[str, list] = {}
        for antenna_id, antenna_masked in self._antenna_mask.items():
        ...
        for smartbox_id in self._smartbox_name_number_map.values():
    ...
    def _get_smartbox_ports_with_antennas(
        self: FieldStationComponentManager, smartbox_id: str
    ) -> list:
    ...
    def turn_on_antenna(
        ...
        :param antenna_id: (one-based) number of the Antenna to turn on.
    ...
    def _turn_on_antenna(  # noqa: C901, pylint: disable=too-many-branches
        ...
        if not ignore_mask and (self._antenna_mask[antenna_id]):
            msg = (
                f"Antenna number {antenna_id} is masked, call "
        ...
        if result is None:
            if task_callback:
                task_callback(
                    status=TaskStatus.COMPLETED,
                    result=f"antenna {antenna_id} was already on.",
                )
        ...
        if result[0] in [
            ...
            if task_callback:
                task_callback(
                    status=TaskStatus.COMPLETED,
                    result=f"turn on antenna {antenna_id} success.",
                )
            return TaskStatus.COMPLETED
        if task_callback:
    ...
    def turn_off_antenna(
        ...
        :param antenna_id: (one-based) number of the TPM to turn on.
        :param task_callback: callback to be called when the status of
            the command changes
    ...
    def _turn_off_antenna(  # noqa: C901
        ...
        if result[0] in [
            ...
            if task_callback:
                task_callback(
                    status=TaskStatus.COMPLETED,
                    result=f"turn off antenna {antenna_id} success.",
                )
            return TaskStatus.COMPLETED
        if task_callback:
            task_callback(status=TaskStatus.FAILED)
    ...
    def _update_antenna_mapping(
        ...
        **kwargs: Any,
        ...
        for antenna_id, values in antenna_mapping.items():
            smartbox_id = values.get("smartboxID")
            smartbox_port = values.get("smartboxPort")
            if (
                antenna_id is not None
                and smartbox_id is not None
                and smartbox_port is not None
            ):
                if antenna_id in self._antenna_mapping:
                    self._antenna_mapping[antenna_id] = (
                        smartbox_id,
                        smartbox_port,
                    )
                else:
                    self.logger.info(f"antenna {antenna_id} not in antenna mapping")
    ...
    def _update_smartbox_mapping(
        ...
        **kwargs: Any,
        ...
        self.logger.error(f"UPDATE SMARTBOX MAPPINGS MAPPING == {smartbox_mapping}")
        for smartbox_id, fndh_port in smartbox_mapping.items():
